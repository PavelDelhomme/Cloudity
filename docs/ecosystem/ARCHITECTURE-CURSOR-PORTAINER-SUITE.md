...
| GasoilTracking | `PavelDelhomme/GasoilTracking` | `…/Perso/GasoilTracking` | `products/fuel` *(à créer)* | `dev` (prod = `prod`) | `gasoil-tracking` | `gasoil_api_data` (~9 Go) |
| YTMusic / PLM | `PavelDelhomme/YTMusic` | `…/Perso/YTMusic` | `products/music` *(à créer)* | `dev` | `ytmusic` | `ytmusic_ytmusic_data` (~21 Go) |
