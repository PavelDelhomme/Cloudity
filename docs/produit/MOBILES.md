...
|---------|------------|-------------|-------------------|--------|
| **Mail** | APP-01 | Oui (prioritaire) | Oui (`mobile/mail`) | Connexion ; multi-boîtes ; dossiers ; **PJ** ; **sync IMAP** ; API comptes expose `last_sync_error` / `imap_auth_ready` (web : alerte sidebar + notif) — **mobile** : afficher bannière si `last_sync_error` (à brancher) ; push système = plus tard |
