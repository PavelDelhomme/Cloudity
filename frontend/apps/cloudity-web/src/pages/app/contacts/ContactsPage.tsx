...
import { ContactRichForm } from './ContactRichForm'
...
export default function ContactsPage() {
  ...
  const [formProfile, setFormProfile] = useState<ContactProfile>(() => emptyContactProfile())
  ...
  const contacts = useMemo(() => {
    return contactsRaw.map((c) => {
      if (!c.vault_encrypted || !c.vault_ciphertext || !contactsVaultScope) {
        return {
          ...c,
          profile: mergeProfile(c.profile, { name: c.name, email: c.email, phone: c.phone }),
        }
      }
      try {
        ...
        return {
          ...
          email: plain.email,
          phone: plain.phone,
          profile: mergeProfile(plain.profile, {
            name: plain.name,
            email: plain.email,
            phone: plain.phone,
          }),
    ...
    const list = !q
      ...
      : contacts.filter((c) => {
          const blob = [
            ...
            c.email,
            c.phone,
            c.profile?.given_name,
            c.profile?.family_name,
            c.profile?.organization,
            c.profile?.nickname,
            ...(c.profile?.emails || []).map((e) => e.value),
            ...(c.profile?.phones || []).map((p) => p.value),
            ...(c.profile?.labels || []),
          ]
            .filter(Boolean)
            .join(' ')
  ...
  const wrapContactPayload = (
    payload: { name?: string; email?: string; phone?: string; profile: ContactProfile },
    idHint: string
  ) => {
    if (!contactsVaultRequired || !contactsVaultScope) return payload
    const email = payload.email || primaryEmail(payload.profile)
    const ciphertext = encryptContactPayload('contacts', contactsVaultScope, idHint, {
      name: payload.name?.trim() || composeDisplayName(payload.profile, email),
      email,
      phone: payload.phone,
      profile: payload.profile,
    })
    return {
      email: email || 'locked@vault.local',
  ...
  const createMutation = useMutation({
    mutationFn: (payload: { name?: string; email?: string; phone?: string; profile: ContactProfile }) =>
      createContact(accessToken!, wrapContactPayload(payload, `new-${Date.now()}`)),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['contacts'] })
      setFormProfile(emptyContactProfile())
      setShowForm(false)
  ...
  const updateMutation = useMutation({
    mutationFn: ({
      ...
      const wrapped =
        contactsVaultRequired && contactsVaultScope
          ? wrapContactPayload(payload, String(id))
          : payload
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    const email = primaryEmail(formProfile)
    if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      toast.error('E-mail invalide')
      return
    }
    const name = composeDisplayName(formProfile, email)
    const phone = primaryPhone(formProfile)
    if (!name && !email && !phone) {
      toast.error('Renseignez au moins un nom, un e-mail ou un téléphone')
      return
    }
    const payload = { name: name || undefined, email: email || undefined, phone: phone || undefined, profile: formProfile }
    if (editingId != null) {
  ...
  const startEdit = (c: ContactResponse) => {
    ...
    setEditingId(c.id)
    setFormProfile(mergeProfile(c.profile, { name: c.name, email: c.email, phone: c.phone }))
    setShowForm(true)
  }
    ...
    <div className="flex flex-col gap-6 min-h-0">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 p-4">
          ...
          <ContactRichForm profile={formProfile} onChange={setFormProfile} />
              ...
              {selected ? (
                <div className="space-y-4">
                    ...
                    <div className="min-w-0">
                      ...
                      {selected.profile?.organization ? (
                        <p className="text-sm text-slate-600 dark:text-slate-300">
                          {[selected.profile.job_title, selected.profile.organization, selected.profile.department]
                            .filter(Boolean)
                            .join(' · ')}
                        </p>
                      ) : null}
                      {selected.email && selected.email !== 'locked@vault.local' ? (
                  ...
                  {selected.profile?.notes ? (
                    <p className="text-sm text-slate-600 dark:text-slate-300 whitespace-pre-wrap rounded-lg bg-white/70 p-3 dark:bg-slate-800/50">
                      {selected.profile.notes}
                    </p>
                  ) : null}
                  {(selected.profile?.labels || []).length > 0 ? (
                    <div className="flex flex-wrap gap-1">
                      {selected.profile!.labels!.map((l) => (
                        <span key={l} className="rounded-full bg-slate-200 px-2 py-0.5 text-xs dark:bg-slate-700">
                          {l}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selected.email && selected.email.includes('@') && selected.email !== 'locked@vault.local' ? (
                      <Link
                    ...
                    <button
                      type="button"
                      onClick={() => requestDelete(selected.id, selected.name.trim() || selected.email || 'contact')}
