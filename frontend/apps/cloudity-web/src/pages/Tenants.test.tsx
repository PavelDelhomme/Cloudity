...
vi.mock('../api', () => ({ fetchTenantsPage: vi.fn(), deleteTenant: vi.fn() }))
...
describe('TenantsPage', () => {
  ...
  it('shows loading then tenants when fetch succeeds', async () => {
    vi.mocked(api.fetchTenantsPage).mockResolvedValue({ items: mockTenants, hasMore: false })
    render(wrap(<Tenants />))
  ...
  it('shows error when fetch fails', async () => {
    vi.mocked(api.fetchTenantsPage).mockRejectedValue(new Error('Network error'))
    render(wrap(<Tenants />))
    await waitFor(() => {
