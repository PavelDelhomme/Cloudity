...
vi.mock('../../api', () => ({ fetchUsersPage: vi.fn(), updateUser: vi.fn() }))
...
describe('UsersPage', () => {
  ...
  it('shows loading then users when fetch succeeds', async () => {
    vi.mocked(api.fetchUsersPage).mockResolvedValue({ items: mockUsers, hasMore: false })
  ...
  it('shows error when fetch fails', async () => {
    vi.mocked(api.fetchUsersPage).mockRejectedValue(new Error('Network error'))
    render(wrap(<UsersPage />))
    await waitFor(() => {
