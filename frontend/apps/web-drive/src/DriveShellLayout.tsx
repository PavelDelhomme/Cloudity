...
/** Nav shell Drive SPA : liens pleine page vers le hub (autres apps = cloudity-web). */
const nav = [
  { name: 'Tableau de bord', href: '/app', icon: LayoutDashboard },
  { name: 'Drive', href: '/app/drive/', icon: HardDrive, current: true },
  ...
  { name: 'Mail', href: '/app/mail/', icon: Mail },
...
/**
 * Layout léger pour la SPA Drive autonome (FE-SPLIT-02).
 * Pas de React Router Link vers /app/* (sinon basename /app/drive casse les URLs).
 */
          ...
          <div className="flex h-dvh bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100">
            ...
            >
              <div className="p-3 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between gap-2">
                <a href="/app/drive/" className={`font-semibold truncate ${open ? '' : 'sr-only'}`}>
                  Hubera
                </a>
                <button
                  type="button"
