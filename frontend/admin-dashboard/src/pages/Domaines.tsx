...
export default function Domaines() {
  ...
  const { data: domains, isLoading, error } = useQuery({
    ...
    queryFn: () => fetchDomains(accessToken!, { skip: page * pageSize, limit: pageSize }),
    enabled: Boolean(accessToken),
  })
  const { data: domainMailboxes = [], isLoading: mailboxesLoading } = useQuery({
    queryKey: ['mail-domain-mailboxes', selectedDomainId, detailsPage, detailsPageSize],
    queryFn: () =>
      fetchDomainMailboxes(accessToken!, selectedDomainId!, {
  ...
  const { data: domainAliases = [], isLoading: aliasesLoading } = useQuery({
    queryKey: ['mail-domain-aliases', selectedDomainId, detailsPage, detailsPageSize],
    queryFn: () =>
      fetchDomainAliases(accessToken!, selectedDomainId!, {
        skip: detailsPage * detailsPageSize,
        limit: detailsPageSize,
      }),
      ...
      {selectedDomainId != null ? (
        <Card>
          ...
          <PaginationControls
            ...
            canNext={(detailsTab === 'mailboxes' ? domainMailboxes.length : domainAliases.length) >= detailsPageSize}
