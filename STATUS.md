...
**Dernière mise à jour** : 2026-05-15 — **Secrets `.env`** : **`MAIL_PASSWORD_ENCRYPTION_KEY`** + **`ALIAS_ENCRYPTION_KEY`** documentés dans **`.env.example`** ; génération **`make secrets`** / **`ensure-mail-encryption-key`** / **`ensure-alias-encryption-key`** ; **`make doctor`** clarifie le succès (✅) vs sync IMAP (ré-enregistrement MDP si rotation). **`docker-compose`** : `ALIAS_ENCRYPTION_KEY` passée au **mail-directory** (futur). Voir **TODOS.md** § `.env`, **SECRETS.md**. *(Conservé : **`make status`** URLs, Pass delete vault, TESTS § 3.5, etc.)*
