...
**Dernière mise à jour** : 2026-05-12 (nuit bis) — **HTTPS partout (cible Zero Trust)** : nouvelles cibles **`make up-tls`** (edge Caddy TLS 1.3 + HSTS + CSP par défaut, hybride PQ X25519MLKEM768 quand dispo) et **`make up-https-internal`** (Postgres TLS `sslmode=verify-ca` + Redis `rediss://` via certs step-ca). Override **`docker-compose.https.yml`** active TLS sur **les 11 services backend** + Postgres + Redis sans casser `make up`. Cibles utilitaires `make mtls-issue-postgres` / `mtls-issue-redis` (TTL 720 h) + `make https-status` (vérifie en-têtes Caddy, `SHOW ssl` Postgres, `PING` Redis TLS). Variables `MTLS_*` documentées dans `.env.example`. Plan de bascule complet dans **[docs/securite/AUDIT-SECURITE.md](docs/securite/AUDIT-SECURITE.md)** § 6 bis.
