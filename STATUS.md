...
**Dernière mise à jour** : 2026-05-18 — **Déploiement** : feuille de route **[DEPLOIEMENT-SUIVI.md](docs/operations/DEPLOIEMENT-SUIVI.md)** (phases A→F : local, Git/CI, Portainer dev/preprod/prod, NPM, Android). Secrets / **IP VPS** : **Portainer uniquement** (pas Git) — **[PORTAINER-DELHOMME-OVH.md](docs/operations/PORTAINER-DELHOMME-OVH.md)** § 0. Hub : **[DEPLOIEMENT-ENVIRONNEMENTS.md](docs/operations/DEPLOIEMENT-ENVIRONNEMENTS.md)**. Monorepo : **[TRAVAIL-MONOREPO-MAINTENANT.md](docs/decisions/multi-repo/TRAVAIL-MONOREPO-MAINTENANT.md)**. **Alias mail (vision produit)** : parcours cible Pass → `*@alias.delhomme.ovh` → Mail (tri, envoi, sans OVH manuel) documenté dans **[docs/produit/MAIL-ALIAS-VISION.md](docs/produit/MAIL-ALIAS-VISION.md)** ; **MVP actuel** = enregistrement + filtre `delivered_to` + envoi `from_email` **sans** provision MX/API (**BACKLOG** **MAIL-ALIAS-01…06**). **Secrets** : **`MAIL_PASSWORD_ENCRYPTION_KEY`** opérationnel ; **`ALIAS_ENCRYPTION_KEY`** en `.env` mais **pas encore lu par le Go** — voir **TODOS.md** § alias + **`make doctor`**. *(Conservé : J8 Pass, **`make status`**, etc.)*
