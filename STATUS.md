...
**Dernière mise à jour (2026-05-12)** — **Bloc 4 questionnaire VPS acté (Q21–Q24)**. **Q21=B** (GHCR) et **Q24=A** (GHA matrice) déjà livrés au commit `349d1642` ; **Q22=A** (réutiliser le bridge external déjà branché à NPM, `<EDGE_NETWORK>`) et **Q23=A** (`cloudity.<DOMAIN>` + `api.` / `admin.`) actés comme défaut. Détails + conséquences DNS / Portainer / NPM dans **[docs/decisions/multi-repo/REPONSES.md](docs/decisions/multi-repo/REPONSES.md) bloc 4**. Tests Go non-régression OK (auth-service 2.0 s, api-gateway 22.6 s, internalsec, mail-directory) ; ESLint cloudity-web clean.
...
| Adminer (profil dev) | 6083 | 8080 | UI **Postgres** dev (`http://localhost:6083`) — équivalent léger de pgAdmin. **Hors prod**. Détail : [`docs/architecture/SERVICES.md`](docs/architecture/SERVICES.md). |
| Redis Commander (profil dev) | 6084 | 8081 | UI **Redis** dev (`http://localhost:6084`, basic auth `admin/admin`) — inspecte refresh tokens, challenges WebAuthn, rate-limit login. **Hors prod**. Détail : [`docs/architecture/SERVICES.md`](docs/architecture/SERVICES.md). |
