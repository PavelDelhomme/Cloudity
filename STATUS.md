...
**Dernière mise à jour** : 2026-05-21 — **Git** : `feat/mail-alias-checklist` fusionné dans **`dev`** (fast-forward `00a0474c`, 9 commits : MTA local, 2FA, alias C1–C6, Pass MP-06/07) ; chantier actif **`feat/mail-alias-prod`** (ENSUITE #6–#7, C7 réel). **Admin polish** : Domaines mail durci contre les réponses `null`, Dashboard perf explicite le fallback cgroup quand Docker n’est pas monté, Users clarifie `Dernière connexion` + statut 2FA sans reset dangereux, CVE priorise les paquets vulnérables, Passkeys/Settings explicitent le périmètre web/mobile/extension ; suivi **TODOS U8/U9**. **Moteur UI** `@cloudity/ui` : `make test-dashboard` vert après correctifs Mail conversations + test Pass alias ; **Mail** : recherche partielle améliorée (`Actu` → `Actualités`), bouton effacer replacé dans la barre, filtres rapides `from:` / `subject:` / `tag:` fiabilisés avec saisie espacée, et validation réelle du message impôts (`dumb@delhomme.ovh`, corps IMAP rechargé) ; suite logique = **ENSUITE** `TODOS.md` #3–#4 (boîte test IMAP, checklist alias) — **[CLOUDITY-UI-DESIGN-SYSTEM.md](docs/architecture/CLOUDITY-UI-DESIGN-SYSTEM.md)** · **Déploiement** : feuille de route **[DEPLOIEMENT-SUIVI.md](docs/operations/DEPLOIEMENT-SUIVI.md)** (phases A→F : local, Git/CI, Portainer dev/preprod/prod, NPM, Android). Secrets / **IP VPS** : **Portainer uniquement** (pas Git) — **[PORTAINER-VPS.md](docs/operations/PORTAINER-VPS.md)** § 0. Hub : **[DEPLOIEMENT-ENVIRONNEMENTS.md](docs/operations/DEPLOIEMENT-ENVIRONNEMENTS.md)**. Monorepo : **[TRAVAIL-MONOREPO-MAINTENANT.md](docs/decisions/multi-repo/TRAVAIL-MONOREPO-MAINTENANT.md)**. **Alias mail (vision produit)** : parcours cible Pass → `*@alias.<domaine-principal>` → Mail (tri, envoi, sans OVH manuel) documenté dans **[docs/produit/MAIL-ALIAS-VISION.md](docs/produit/MAIL-ALIAS-VISION.md)** ; **MVP actuel** = enregistrement + filtre `delivered_to` + envoi `from_email` **sans** provision MX/API (**BACKLOG** **MAIL-ALIAS-01…06**). **Secrets** : **`MAIL_PASSWORD_ENCRYPTION_KEY`** opérationnel ; **`ALIAS_ENCRYPTION_KEY`** en `.env` mais **pas encore lu par le Go** — voir **TODOS.md** § alias + **`make doctor`**. *(Conservé : J8 Pass, **`make status`**, etc.)*
...
1. **Git** : `git status` ; branche = **`feat/mail-alias-prod`** (DNS/Maddy prod, C7) — **[docs/GIT.md](docs/GIT.md)** + **[docs/operations/BRANCHES.md](docs/operations/BRANCHES.md)**.
...
3. **ENSUITE — Mail** : MTA alias livré sur **`dev`** (API `/mail/internal/alias-resolve`, Maddy local, admin Domaines) ; sur **`feat/mail-alias-prod`** : DNS/MX/SPF/DKIM/DMARC prod, C7 réel (réception IMAP / redirection fournisseur), secrets VPS/Portainer avant mise en ligne.
