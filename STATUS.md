...
**Dernière mise à jour** : 2026-05-12 — durcissement **admin** : UI **`/admin*`** refusée (pas de redirection vers `/4dm1n`) ; gateway **`/admin/*`** exige **JWT admin + `Origin`** ; `POST /admin/performance/pipeline-run` exige **JWT + `X-Cloudity-Perf-Ingest`** (`PERFORMANCE_INGEST_TOKEN` **obligatoire** gateway+admin-service) ; restauration des guides `docs/{architecture,operations,produit,securite}` + audit renommé **`docs/securite/AUDIT-SECURITE.md`**. *(Historique ci-dessous.)*
