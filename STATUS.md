...
**Dernière mise à jour** : 2026-05-13 — **Sprint Pass + gel multi-repo**. Décision produit : **priorité absolue** au chemin critique **Pass** (crypto client `EnvelopeV1`, UI Pass complète, import Proton JSON, TOTP item, 2FA compte Cloudity + codes de récupération) pour une **migration utilisable ~20 mai 2026** (marge avant fin abonnement Proton ~25 mai). **Extension navigateur MV3** en L2 ; **mobile Flutter `pass`** en L3. **Scission du monorepo en plusieurs dépôts GitHub** : **gelée** jusqu’à fin du sprint — le dépôt unique reste la référence. Fiche **[docs/produit/SPRINT-PASS-2026-05.md](docs/produit/SPRINT-PASS-2026-05.md)** + section correspondante dans **[BACKLOG.md](BACKLOG.md)**. État technique rappel : `passwords-service` + spec **PASS-CRYPTO** solides ; **PassPage** encore stub ; **TOTP backend** (`/auth/2fa/*`) partiellement branché — **login web 2FA non finalisé** ; pas de codes de récupération.
