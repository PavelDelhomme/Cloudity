...
**Dernière mise à jour** : 2026-05-12 — **REVERSE-PROXY.md § 4 bis nginx-proxy-manager**. Section dédiée au scénario VPS Contabo actuel (NPM `nginx.delhomme.ovh`) : table des 3 Proxy Hosts (`api.` / `app.` / `admin.cloudity.delhomme.ovh`), blocs « Advanced » prêts à coller (HSTS preload, nosniff, X-Frame-Options, Permissions-Policy, COOP/CORP, CSP API stricte / front avec `connect-src` API, rate limit edge, cache long `/assets/*`), table des limites NPM vs Caddy/nginx natif (HTTP/3 Q18=A et PQ X25519MLKEM768 Q19=A à activer après bascule Caddy planifiée), tests rapides via `make smoke-prod`. Aligné Q21–Q24.

**Dernière mise à jour (2026-05-12)** — **`make smoke-prod` post-déploiement**. `scripts/ops/smoke-prod.sh` : `GET /health` (200) + `GET /auth/validate` sans Bearer (401) + front SPA (200) + TLS handshake (proto/cipher) + headers durcis (HSTS + nosniff). Authentifié optionnel via `SMOKE_USER`/`SMOKE_PASS` (login → 4 endpoints applicatifs). Aucun secret embarqué. Cible `make smoke-prod` (`SMOKE_API_URL=https://api.cloudity.delhomme.ovh make smoke-prod`).

**Dernière mise à jour (2026-05-12)** — **Volume `cloudity_auth_keys` persistant (prod)**. `auth-service` accepte `AUTH_KEYS_DIR` (helper `keyDir()` + `keyPath()`) ; les paires Ed25519 (clé courante) + RSA (rétrocompat RS256) sont écrites/relues dans ce répertoire. `docker-compose.prod.yml` monte le volume nommé `cloudity_auth_keys` à `/var/lib/cloudity/auth-keys` côté auth-service (RW) et côté api-gateway en RO (`JWT_PUBLIC_KEY_PATH` + `JWT_ED25519_PUBLIC_KEY_PATH`). Plus aucun JWT cassé au redéploiement. Tests `TestKeyDirOverrideWritesAndReloadsEd25519` / `…RSA` / `TestKeyPathCreatesDirectory` ✅ (suite auth-service 2.8 s).
