...
| M2 | **Frontend Mail** | **En place** : liste messages, **sync IMAP** + **polling** arrière-plan (intervalle configurable), **Menu Mail** (fil d’Ariane) : paramètres app, Google OAuth, ajout boîte, **Actualiser (IMAP)** ; Paramètres panneau : « Sync maintenant » / « Sync avec mot de passe… » (**docs/PLAN.md §9**), notifications nouveaux messages, panneau gauche réductible (comptes repliés, dossiers + sous-dossiers IMAP), réponse / transfert, rédaction panneau bas, signature localStorage, envoi sans ressaisir mot de passe, multi-sélection + actions masse, pagination `Page X / Y`, sélection page / inverser, **sélection boîte entière** (`Tout sélectionner (boîte)`) avec récupération de tous les IDs du périmètre courant, et actions de masse **optimisées par lot** (une requête batch par compte pour `read` / `folder`). **Tests backend batch** ajoutés (`mail-directory-service/main_test.go`) + **tests frontend MailPage** alignés sur les endpoints batch (corbeille/archive/lu). **À ajouter** : raffinements UX, corps à l’ouverture si absent en base (cf. M8). | Unit : MailPage ; E2E : ajout boîte, sync, envoi. |
