...
| M7 | **Spam** | **Partiel (MVP UI)** : dossier Spam IMAP sync ; marquer spam / **Pas indésirable** (→ inbox) ; score heuristique `spam_score` + icône en réception (≥ 52) ; bannière explicative dans le dossier Spam. **À faire (AS-1)** : stack **Rspamd** + scoring MTA, règles auto, SPF/DKIM/DMARC — voir **BACKLOG AS-1**, **ANTI-SPAM-ET-ABUS.md**. | API : folder spam ; tests MailPage spam batch + score. |
