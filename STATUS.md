...
**Dernière mise à jour** : 2026-04-30 (convention doc : racine **README** + **STATUS** + **BACKLOG.md** ; ROADMAP / TESTS / MOBILES / PlanImplementation / guides → **`docs/`**). **§0b A1** : workspaces npm racine **`frontend/`** (détail tableau ci-dessous). **Mail** : barre de page allégée (**Nouveau message** seul) ; comptes / Google / paramètres app / synchro dans **« Menu Mail »** à côté du fil d’Ariane (**`AppPageChromeContext`** — réutilisable Calendar, etc.). **Mise à jour avril 2026** : suppression définitive en masse des messages sélectionnés depuis la corbeille (API + UI), neutralisation temporaire du bridge AppChrome dans `MailPage` pour casser la boucle `Maximum update depth`, retour des favicons via **proxy/cache backend Cloudity** (plus d’appels tiers directs depuis le navigateur), **réconciliation IMAP après règles** (move + lu/non lu), **anti-rafale sync auto** côté UI (batch unique, anti-chevauchement, cadence minimale, pause onglet caché) et **indicateur visuel de sync auto** dans la sidebar Mail. **Migrations** : voir tableau §0 (**`make migrate`**) et **TESTS.md** / **SYNC-BACKLOG §0d**. **Performances** : **[docs/PERFORMANCES.md](./docs/PERFORMANCES.md)** + **BACKLOG** n°9 + **ROADMAP TR-06** ; gros exports de trace ignorés par **`.gitignore`**.  
...
| M2 | **Frontend Mail** | **En place** : liste messages, **sync IMAP** + **polling** arrière-plan (intervalle configurable), **Menu Mail** (fil d’Ariane) : paramètres app, Google OAuth, ajout boîte, **Actualiser (IMAP)** ; Paramètres panneau : « Sync maintenant » / « Sync avec mot de passe… » (**docs/PLAN.md §9**), notifications nouveaux messages, panneau gauche réductible (comptes repliés, dossiers + sous-dossiers IMAP), réponse / transfert, rédaction panneau bas, signature localStorage, envoi sans ressaisir mot de passe, multi-sélection + actions masse, pagination `Page X / Y`, sélection page / inverser, **sélection boîte entière** (`Tout sélectionner (boîte)`) avec récupération de tous les IDs du périmètre courant, et actions de masse **optimisées par lot** (une requête batch par compte pour `read` / `folder`). **Tests backend batch** ajoutés (`mail-directory-service/main_test.go`) + **tests frontend MailPage** alignés batch sur les actions de masse (corbeille, archive, lu, non lu, spam, remise en réception). **À ajouter** : raffinements UX, corps à l’ouverture si absent en base (cf. M8). | Unit : MailPage ; E2E : ajout boîte, sync, envoi. |
...
| M8 | **Paramètres et conversations** | Paramètres Mail : **signature** (déjà en place). UX compte mail (sync) : **livré** — champs **serveur IMAP/SMTP verrouillés** pendant la synchronisation de la boîte en cours + message explicite dans la modale ; enregistrement bloqué durant ce verrou. **Sync par boîte** (boutons dédiés) : livré — **PLAN §9**. À venir : **règles** par boîte. Conversations : grouper mails d'un même fil (thread). **Corps du message** : récupération à l’ouverture **livrée** (fallback IMAP `BODY.PEEK[]` si corps absent **ou vide** en base, puis persistance backend). **Notifications mail en arrière-plan** (sans onglet ouvert) : PWA / Service Worker ou polling léger à définir. **Brouillons** : récupération et envoi à gérer (côté app mail). | Unit : paramètres, thread. |
