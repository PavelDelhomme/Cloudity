...
const Key k2faCode = ValueKey('cloudity_photos_login_2fa_code');
...
void main() {
  ...
  testWidgets(
    'login compte 2FA : mot de passe puis TOTP → Photos',
    (tester) async {
      await tester.pumpWidget(const app.CloudityPhotosApp());
      for (var i = 0; i < 80; i++) {
        await tester.pump(const Duration(milliseconds: 250));
        if (_onLogin(tester) || _on2FA(tester) || _onTimeline(tester)) break;
      }

      if (_onTimeline(tester)) return;
      ...
      for (var i = 0; i < 120; i++) {
        ...
        if (_on2FA(tester) || _onTimeline(tester)) break;
      }

      if (!_onTimeline(tester)) {
        expect(_on2FA(tester), isTrue);
