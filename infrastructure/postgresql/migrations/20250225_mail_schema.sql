-- Migration : schéma Mail (identique à init/03-schema-mail.sql)
-- À appliquer si la base existait avant l'ajout de 03-schema-mail.sql (init non réexécuté).
-- Usage : make migrate-mail  ou  psql -h localhost -p 6042 -U cloudity_admin -d cloudity -f infrastructure/postgresql/migrations/20250225_mail_schema.sql
...
CREATE INDEX IF NOT EXISTS idx_mail_domains_tenant ON mail_domains(tenant_id);
...
DROP POLICY IF EXISTS mail_domains_tenant_isolation ON mail_domains;
...
DROP POLICY IF EXISTS mail_mailboxes_via_domain ON mail_mailboxes;
...
DROP POLICY IF EXISTS mail_aliases_via_domain ON mail_aliases;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_mail_domains_updated_at') THEN
    CREATE TRIGGER update_mail_domains_updated_at BEFORE UPDATE ON mail_domains
      FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_mail_mailboxes_updated_at') THEN
    CREATE TRIGGER update_mail_mailboxes_updated_at BEFORE UPDATE ON mail_mailboxes
      FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_mail_aliases_updated_at') THEN
