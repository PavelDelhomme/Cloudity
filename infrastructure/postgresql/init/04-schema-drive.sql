-- Schema Drive : dossiers et fichiers en cascade (type Google Drive / Nextcloud)
-- parent_id NULL = racine. Isolation par user_id + tenant_id.
...
CREATE UNIQUE INDEX idx_drive_nodes_unique_child ON drive_nodes(user_id, parent_id, name) WHERE parent_id IS NOT NULL;
CREATE UNIQUE INDEX idx_drive_nodes_unique_root ON drive_nodes(user_id, name) WHERE parent_id IS NULL;
CREATE INDEX idx_drive_nodes_user_parent ON drive_nodes(user_id, parent_id);
CREATE INDEX idx_drive_nodes_tenant ON drive_nodes(tenant_id);
