...
# 3. Frontend : ne pas écraser le monorepo ; scaffold minimal uniquement si absent
echo "📦 Configuration du frontend..."
if [ -f "frontend/apps/cloudity-web/package.json" ] || { [ -f "frontend/package.json" ] && grep -q "workspaces" frontend/package.json 2>/dev/null; }; then
    echo "✅ Frontend monorepo / @cloudity/web déjà présent — pas de scaffold minimal"
elif [ ! -f "frontend/apps/cloudity-web/package.json" ]; then
    mkdir -p frontend/apps/cloudity-web/src
    cat > frontend/apps/cloudity-web/package.json << 'EOF'
{
  "name": "@cloudity/web",
  "private": true,
  "version": "0.0.0",
...
EOF
    echo "✅ package.json frontend créé (apps/cloudity-web)"
fi

# 4. Créer un App.tsx minimal si nécessaire
if [ ! -f "frontend/apps/cloudity-web/src/App.tsx" ]; then
    mkdir -p frontend/apps/cloudity-web/src
    cat > frontend/apps/cloudity-web/src/App.tsx << 'EOF'
...
if [ ! -f "frontend/apps/cloudity-web/src/main.tsx" ]; then
    cat > frontend/apps/cloudity-web/src/main.tsx << 'EOF'
import React from 'react'
...
if [ ! -f "frontend/apps/cloudity-web/index.html" ]; then
    cat > frontend/apps/cloudity-web/index.html << 'EOF'
<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
