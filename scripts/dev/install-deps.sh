...
# Utilisé par : make install, et par scripts/dev/setup.sh
...
for dir in backend/auth-service backend/api-gateway backend/calendar-service \
  ...
  backend/passwords-service backend/mail-directory-service; do
...
# Node — workspaces à la racine frontend/ si présent, sinon app cloudity-web seule
if [ -f frontend/package.json ]; then
  echo "  npm: frontend/ (workspaces)"
  (cd frontend && npm install)
elif [ -f frontend/apps/cloudity-web/package.json ]; then
  echo "  npm: frontend/apps/cloudity-web"
  (cd frontend/apps/cloudity-web && npm install)
fi
if [ -f frontend/package.json ] || [ -f frontend/apps/cloudity-web/package.json ]; then
  echo "✅ npm deps OK"
fi
