...
# .env
if [ ! -f .env ]; then
    echo "❌ Fichier .env manquant. Lancez: make create-env ou scripts/dev/setup.sh"
    exit 1
fi
...
elif command -v node &>/dev/null && [ -f frontend/apps/cloudity-web/package.json ]; then
    echo "📦 Node: npm install (frontend workspaces / @cloudity/web)..."
    (cd frontend && npm install)
fi
