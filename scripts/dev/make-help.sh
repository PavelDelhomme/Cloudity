...
printf "  %-${WIDTH}s %s\n" "make up-ready" "Stack locale + seed admin/user (quotidien, sans tests)"
printf "  %-${WIDTH}s %s\n" "make up-full" "stack+seed+tests (Pass soft ; tests KO ≠ bloque la stack)"
...
printf "  %-${WIDTH}s %s\n" "make test-mobile-avd" "Suite Photos→Drive→Mail sur AVD (stack+émulateur auto)"
printf "  %-${WIDTH}s %s\n" "make test-mobile-avd-drive" "Tests Drive uniquement sur AVD Cloudity"
...
echo "  CLOUDITY_AVD_USE_SYSTEMD=1 # lancement qemu via systemd-run (stable hors TTY)"
echo "  CLOUDITY_AVD_SKIP_STACK=1  # test-mobile-avd sans toucher Docker"
echo "  UP_FULL_SKIP_TESTS=1      # up-full sans la batterie de tests"
echo "  UP_FULL_REQUIRE_TESTS=1   # up-full échoue si les tests échouent (défaut: stack OK = exit 0)"
echo ""
