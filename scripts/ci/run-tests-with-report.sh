...
# Usage: ./scripts/ci/run-tests-with-report.sh  ou  make tests
...
echo ""
echo ">>> Phase 5/5 : Mobile Cloudity (./scripts/mobile/test-mobile-suite.sh)"
MOBILE_PHOTOS_STATUS="OK"
if run_phase "Phase 5: test-mobile-suite" "chmod +x scripts/mobile/test-mobile-suite.sh scripts/mobile/test-mobile-app.sh scripts/mobile/test-mobile-mail.sh 2>/dev/null || true; ./scripts/mobile/test-mobile-suite.sh"; then
  echo ""
...
if [ "$SEC_STATUS" = "OK (avertissements)" ]; then
  ...
  echo "   Vulnérabilités : npm audit (service cloudity-web), govulncheck (services Go). Détails dans le rapport."
  echo "   Rapport : $LOG"
  echo "   Vous êtes toujours dans la racine du dépôt."
else
  echo ""
