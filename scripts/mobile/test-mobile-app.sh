...
chmod +x "${ROOT}/scripts/mobile/check-flutter-sdk-writable.sh" 2>/dev/null || true
...
if ! CLOUDITY_ALLOW_READONLY_FLUTTER_SDK=1 CLOUDITY_QUIET_FLUTTER_SDK_CHECK=1 "${ROOT}/scripts/mobile/check-flutter-sdk-writable.sh"; then
  echo ""
