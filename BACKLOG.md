...
- [ ] **mTLS strict** : passer chaque lien `gateway → service` en `MTLS_MODE=strict` (séquence prévue : auth-service → mail-directory → autres ; admin-service est déjà cert-aware en `permissive`).
