...
- [ ] J5 — **2FA compte Cloudity** : aujourd’hui `POST /auth/2fa/enable` + `POST /auth/2fa/verify` ✅ mais **login web non branché** ; **codes de récupération** absents → migration SQL (table dédiée bcrypt) + API `enable/verify/regenerate/use` + UI Settings. **+ Passkeys backend** : ouvrir l'enrôlement aux **comptes user** (aujourd'hui réservé admin, cf. `backend/auth-service/webauthn.go` ligne 161), forcer **`residentKey: required` + `userVerification: preferred`** (sans ça les password managers tiers — Proton Pass / Bitwarden / iCloud Keychain — n'enregistrent pas la passkey), endpoint `POST /auth/webauthn/login/begin-discoverable` (challenge sans email préalable, résolution user via `userHandle`), quota 5 passkeys / user.
- [ ] J6 — **Login 2FA web étape 2** + **Settings 2FA** (QR `otpauth://`, secret manuel, vérification, affichage codes de récupération une fois). **+ Settings Passkeys user** (réutilise composant `Passkeys.tsx` admin, branché `/app/settings/security/passkeys`) **+ Conditional UI sur `LoginPage`** : `autocomplete="username webauthn"` + `navigator.credentials.get({ mediation: 'conditional' })` → le PM tiers (Proton Pass migré, ou Bitwarden / iCloud Keychain) **propose la passkey au focus du champ email**, comme sur GitHub / Google.
