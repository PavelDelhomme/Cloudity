...
- [x] **HTTPS par défaut (edge + interne)** : nouvelles cibles **`make up-tls`** (stack + Caddy edge `https://app.cloudity.local` / `https://api.cloudity.local`) et **`make up-https-internal`** (Postgres TLS + Redis TLS via step-ca, DSN en `sslmode=verify-ca`, cache en `rediss://`). Override **`docker-compose.https.yml`** + script init **`infrastructure/postgresql/tls-init.sh`**. Cibles `mtls-issue-postgres` / `mtls-issue-redis` (TTL 720 h) + `https-status` pour vérifier l'activation. Section **`AUDIT-SECURITE.md` § 6 bis** documente l'état réel + plan de bascule.
- [ ] **Faire de `make up-tls` l'alias de `make up`** (HTTPS par défaut) — après 1 sprint stable de surveillance.
- [ ] **mTLS strict** : passer chaque lien `gateway → service` en `MTLS_MODE=strict` (séquence prévue : admin-service → auth-service → mail-directory → autres).
- [ ] **Sidecar `step ca renew`** par service (rotation auto 24 h en dev, 7 j en prod).
- [ ] **Postgres `sslmode=verify-full`** + cert client par service (`auth_app`, `pass_app`, …) après stabilisation `verify-ca`.
- [ ] **Redis `--tls-auth-clients yes`** + cert client par service après vérification `go-redis` `rediss://`.
