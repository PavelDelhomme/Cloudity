...
- [ ] **Mail web — stabilité React** : supprimer définitivement le warning `Maximum update depth exceeded` (audit `useEffect` + dépendances stables + garde anti-setState), avec scénario manuel de navigation inter-app (`/app/mail` ↔ `/app/drive`) et capture console propre.
